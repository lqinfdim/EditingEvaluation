...
import numpy as np
import random
from ..methods.melo.melo import LORA
...
class BaseEditor:
    ...
    def __init__(self,
        ...
        hf_token = "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",
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                torch_dtype = torch.float16 if self.alg_name == "MEND" else torch.float32
                self.model = LlamaForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map=device_map, use_auth_token=hf_token)
                ...
                pythia_dtype = torch.float16 if "12b" in self.model_name.lower() else torch.float32
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=pythia_dtype, device_map=device_map)
                self.tok = AutoTokenizer.from_pretrained(self.model_name)
                self.tok.pad_token_id = self.tok.eos_token_id
            else:
    ...
    def edit_dataset(self,
        ...
        if not keep_original_weight:
            print("[INFO] Sequential Editing for Dataset. ")
        ...
        print(f"[INFO] The num of edits is {num_edits} . \n")
